...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':40} | {'GPT-4o':40} | {'Mini':40} | GPT4o Lat | Mini Lat"
    lines.append(header)
    ...
    for r in results:
        ...
        g_lat = f"{r['gpt4o_latency']:.2f}"
        m_lat = f"{r['mini_latency']:.2f}"

        line = f"{prompt:40} | {gpt:40} | {mini:40} | {g_lat:8} | {m_lat:8}"
